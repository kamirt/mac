<template lang="pug">
  uni-carousel(:perPage="alternative ? 4.5 : 6.8", ref="vehicleCarousel")
    div(class="slide nav-item", v-for="(item, key) in vehicles",
    :key="item.id",
    :class="selectedItem.id == item.id ? 'selected' : ''",
    @click="(e) => handleMouseUp(e, item)",
        ...
        div(class="vehicle", :class="{ alternative: alternative }")
          ...
          transition(name="fade", appear)
            div(class="vehicle__pic", :class="{ blocked: item.blocked, 'buy-only': item['buy_only'] }")
              ...
              img(:src="getVehiclePic(item)", :class="hoveredItem == item.id ? 'full-opacity' : null")
...
export default {
  ...
  watch: {
    'vehicles.length' (next, prev) {
      if (next > 0 && !prev) {
        this.$emit('loaded', this.vehicles[0])
      }
    }
  },
  data () {
